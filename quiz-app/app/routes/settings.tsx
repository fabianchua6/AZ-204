...
export default function Settings() {
		...
		for (const topic of topics) {
			...
			if (progress && progress.questionIds.length > 0) {
				topicData.push({
					...
					percentage: Math.round((progress.index / progress.questionIds.length) * 100),
				});
			}
		}
	...
	const handleClearTopicProgress = useCallback((topic: string) => {
	...
	const handleClearRandomProgress = useCallback((topic: string | null) => {
				...
				{topicProgress.length > 0 ? (
					<ul className="divide-y divide-gray-100">
						{topicProgress.map((item) => (
							<li key={item.topic} className="flex items-center justify-between py-3">
