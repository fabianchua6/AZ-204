...
const createMockQuestions = (count: number): Question[] => {
  return Array.from({ length: count }, (_, i) =>
    createMockQuestion(
      `question-${i.toString().padStart(3, '0')}`,
      `Topic ${i % 5}`
    )
  );
};
...
describe('Leitner Question Selection Improvements', () => {
  ...
  test('getDueQuestions returns different orders with seed rotation', async () => {
    ...
    const differentPositions = order1.filter(
      (q, index) => index < order2.length && order2[index].id !== q.id
    ).length;
  ...
  test('includes questions from different boxes for variety', async () => {
    ...
    const boxCounts = dueQuestions.reduce(
      (acc, q) => {
  ...
  test('includes box 3 questions for review with increased probability', async () => {
    ...
    // Since we have 25 naturally due questions (> 20 minimum) and probability is now 0%,
    // box 3 questions should NEVER be included randomly
    ...
    // Should not include box 3 questions at all
    const inclusionRate = box3Inclusions / trials;
    expect(inclusionRate).toBe(0); // 0% of trials
  });
  ...
  test('topic interleaving distributes questions evenly', async () => {
    ...
    const topicQuestions = [
      ...Array.from({ length: 10 }, (_, i) =>
        createMockQuestion(`topic-a-${i}`, 'Topic A')
      ),
      ...Array.from({ length: 10 }, (_, i) =>
        createMockQuestion(`topic-b-${i}`, 'Topic B')
      ),
      ...Array.from({ length: 10 }, (_, i) =>
        createMockQuestion(`topic-c-${i}`, 'Topic C')
      ),
    ];
  ...
  test('prioritizes questions with more failures', async () => {
    ...
    const dueQuestions = await leitnerSystem.getDueQuestions([
      questionA,
      questionB,
    ]);
