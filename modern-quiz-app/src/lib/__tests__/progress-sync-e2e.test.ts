...
function seedRealisticProgress() {
    ...
    localStorageMock.setItem(
        ...
        JSON.stringify({ index: 12, questionOrder: ['q1', 'q2', 'q3'], timestamp: Date.now() })
    );
    localStorageMock.setItem(
        'quiz_progress_blob-storage',
        JSON.stringify({ index: 5, questionOrder: ['q10', 'q11'], timestamp: Date.now() })
    );
...
function mockFetchRoundTrip() {
    ...
    (global.fetch as jest.Mock).mockImplementation(async (url: string, init?: RequestInit) => {
        if (init?.method === 'POST') {
            ...
            return {
                ok: true,
                json: async () => ({ success: true, lastSync, message: 'Data synced successfully' }),
            };
        }
        // GET
        return {
...
describe('Progress Collection', () => {
    it('collects all quiz progress keys into quizProgress', () => {
        ...
        expect((data.quizProgress['quiz_progress_azure-functions'] as { index: number }).index).toBe(12);
    });
    ...
    it('collects answered questions with all topic keys', () => {
        ...
        expect((data.answeredQuestions as Record<string, string[]>)['azure-functions']).toEqual([
            'q1', 'q2', 'q3',
        ]);
    });
    ...
    it('collects all leitner-prefixed keys', () => {
        ...
        expect(data.leitnerProgress).toHaveProperty('leitner-daily-attempts-2026-02-18');
    });
    ...
    it('preserves full Leitner box assignments per question', () => {
        ...
        const lp = data.leitnerProgress['leitner-progress'] as Record<string, { currentBox: number }>;
    ...
    it('skips malformed JSON values gracefully', () => {
        ...
        localStorageMock.setItem('quiz_progress_good', JSON.stringify({ index: 1 }));
...
describe('Sync Round-Trip', () => {
    ...
    it('Leitner box assignments survive a round-trip', async () => {
        ...
        (global.fetch as jest.Mock).mockResolvedValueOnce({
            ok: true,
            json: async () => ({ success: true, data: { ...sentBody, lastSync }, lastSync }),
        });
    ...
    it('preserves answered question lists across devices', async () => {
        ...
        const answeredCall = (localStorageMock.setItem as jest.Mock).mock.calls.find(
            (c: string[]) => c[0] === 'quiz_answered_global'
        );
    ...
    it('stores sync code and last sync time after push', async () => {
        ...
        expect(localStorageMock.setItem).toHaveBeenCalledWith('quiz_sync_code', syncCode);
        expect(localStorageMock.setItem).toHaveBeenCalledWith(
            'quiz_last_sync',
    ...
    it('stores sync code and last sync time after pull', async () => {
        ...
        expect(localStorageMock.setItem).toHaveBeenCalledWith('quiz_sync_code', syncCode);
        expect(localStorageMock.setItem).toHaveBeenCalledWith('quiz_last_sync', lastSync);
    });
});
...
describe('Edge Cases', () => {
    it('push works with completely empty localStorage', async () => {
        (global.fetch as jest.Mock).mockResolvedValueOnce({
            ok: true,
            json: async () => ({ success: true, lastSync: '2026-02-18T12:00:00.000Z' }),
        });
    ...
    it('pull throws on 500 response', async () => {
        (global.fetch as jest.Mock).mockResolvedValueOnce({
            ok: false,
        ...
        const syncCode = generateSyncCode();
        await expect(pullData(syncCode)).rejects.toThrow('Pull failed');
    });
    ...
    it('handles large payloads (many topics, many questions)', async () => {
        ...
        for (let i = 0; i < 50; i++) {
            localStorageMock.setItem(
                ...
                JSON.stringify({ index: i, questionOrder: Array.from({ length: 100 }, (_, j) => `q${j}`) })
            );
        }
        ...
        const lp = data.leitnerProgress['leitner-progress'] as Record<string, unknown>;
