...
export function useLeitnerSession({ questions, onSessionReset }: UseLeitnerSessionProps) {
    const [filteredQuestions, setFilteredQuestions] = useState<Question[]>([]);
    const [isLoadingSession, setIsLoadingSession] = useState(true);
    const [savedSessionResults, setSavedSessionResults] = useState<SessionResults | null>(null);
        ...
        const updateQuestions = async () => {
            ...
            try {
                ...
                if (savedSession &&
                    ...
                    if (allAnswered) {
                        debug('⚠️ All questions answered in saved session - generating fresh session');
    ...
    const endCurrentSession = useCallback((
        currentSubmissions: Record<string, { isSubmitted: boolean, isCorrect: boolean }>
    ) => {
