...
export function useSmartLearning(questions: Question[]) {
	const [learningRecords, setLearningRecords] = useState<Map<string, LearningRecord>>(new Map());
	...
	useEffect(() => {
		try {
			...
			if (savedRecords) {
				const recordsArray: [string, any][] = JSON.parse(savedRecords);
	...
	useEffect(() => {
		if (learningRecords.size > 0) {
			const recordsArray = Array.from(learningRecords.entries()).map(([id, record]) => [
				id,
				{
					...record,
			...
			localStorage.setItem(STORAGE_KEYS.LEARNING_RECORDS, JSON.stringify(recordsArray));
		}
	}, [learningRecords]);
	...
	const calculateNextReview = useCallback((record: LearningRecord, wasCorrect: boolean): Date => {
		const now = new Date();
		let intervalDays = 1;
		...
		if (wasCorrect) {
			...
			switch (record.streak) {
				...
				case 3: intervalDays = 14; break;
				case 4: intervalDays = 30; break;
				default: intervalDays = Math.min(90, 30 * Math.pow(1.5, record.streak - 4));
			}
		} else {
		...
		setLearningRecords(prev => {
			...
			const record: LearningRecord = existing ? {
				...
				confidence: Math.max(0, Math.min(1, 
					existing.confidence + (wasCorrect ? 0.1 : -0.2)
				)),
			} : {
				questionId,
				attempts: 1,
	...
	const getSmartQuestionOrder = useCallback((questions: Question[]): Question[] => {
		const now = new Date();
		...
		const categorized = questions.map(q => {
			...
			return {
				...
				isNew: !record,
				isMastered: record ? record.streak >= 3 && record.confidence >= 0.8 : false,
				isStruggling: record ? record.correctCount / record.attempts < 0.5 : false,
	...
	const getLearningMetrics = useCallback((): LearningMetrics => {
		...
		const masteredQuestions = records.filter(r => 
			r.streak >= 3 && r.confidence >= 0.8
		).length;
		
		const strugglingQuestions = records.filter(r => 
			r.attempts >= 2 && r.correctCount / r.attempts < 0.5
		).length;
		
		const now = new Date();
		const reviewDueQuestions = records.filter(r => 
			r.nextReviewDate <= now && r.streak < 3
		).length;
		
		const averageConfidence = answeredQuestions > 0 
	...
	const endSession = useCallback(() => {
		if (currentSession) {
			const endedSession = {
				...currentSession,
				endTime: new Date(),
				averageTimePerQuestion: currentSession.questionsAnswered > 0 
					? (Date.now() - currentSession.startTime.getTime()) / (currentSession.questionsAnswered * 1000)
					: 0,
			};
