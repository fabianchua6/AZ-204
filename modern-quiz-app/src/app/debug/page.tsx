...
export default function DebugPage() {
  ...
  const clearQuizNavigationStates = () => {
    try {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key && (key.startsWith('submission-') || key.startsWith('card-state-'))) {
          localStorage.removeItem(key);
          clearedCount++;
        }
      }
      
      setClearResult(`✅ Cleared ${clearedCount} navigation state items. Your Leitner progress is preserved. Please refresh the page.`);
    } catch (error) {
  ...
  const clearApplicationCache = async () => {
    try {
      ...
      if (itemsToRemove.length > 0) {
        clearedItems.push(`localStorage: ${itemsToRemove.length} non-essential items`);
      }
  ...
  const showStorageDebug = () => {
    ...
    setClearResult('📋 localStorage contents logged to console (F12 > Console)');
  };
      ...
      <div className='mx-auto max-w-4xl space-y-8'>
        ...
        {!isMounted ? (
          <div className="flex items-center justify-center p-8">
            <div className="text-muted-foreground">Loading debug information...</div>
          </div>
        ) : (
          <>
            {/* Timezone Debug */}
        ...
        <section className='space-y-4'>
          ...
          <Card className="p-4">
            <h3 className="font-semibold mb-4">Clear Navigation States</h3>
            <p className="text-sm text-muted-foreground mb-4">
              If you're experiencing issues with quiz navigation (can't go back/forward, answers not showing), 
              click below to clear navigation states. This preserves your Leitner progress and stats.
            </p>
        ...
        <section className='space-y-4'>
          ...
          <Card className="p-4">
            <h3 className="font-semibold mb-4">Clear Application Cache</h3>
            <p className="text-sm text-muted-foreground mb-4">
              If you're seeing old styling or the app isn't reflecting recent updates, clear the cache below.
              Your Leitner progress and question stats will be preserved.
            </p>
            <div className="space-y-3">
              <Button 
              ...
              <div className="text-xs text-muted-foreground space-y-1">
                <p><strong>Manual alternatives:</strong></p>
                <p>• Mac: Cmd + Shift + R (hard refresh)</p>
                <p>• Windows/Linux: Ctrl + Shift + R (hard refresh)</p>
                <p>• Or open DevTools (F12) → Network tab → check "Disable cache"</p>
              </div>
            </div>
          </Card>
        </section>
