...
function analyzeQuestion(content, startLine, endLine, filename) {
  ...
  const answerLines = questionContent
    .split('\n')
    .filter(line => line.trim().startsWith('- ['));
  const answerTexts = answerLines.map(line =>
    line.replace(/- \[[x ]\]\s*/, '').trim()
  );
  const duplicates = answerTexts.filter(
    (item, index) => answerTexts.indexOf(item) !== index
  );
  ...
  const correctAnswers = answerLines.filter(line =>
    line.includes('[x]')
  ).length;
  if (correctAnswers > 4) {
    issues.push({
